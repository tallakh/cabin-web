...
export async function PATCH(
  ...
  try {
    ...
    const datesChanged =
      (start_date && start_date !== existingBooking.start_date) ||
      (end_date && end_date !== existingBooking.end_date);
    const guestsChanged =
      number_of_guests !== undefined &&
      number_of_guests !== (existingBooking.number_of_guests || 1);
    ...
    if (status && !isAdmin) {
      return NextResponse.json(
        { error: "Only admins can change booking status" },
        { status: 403 },
      );
    }
    ...
    if (cabin_id && cabin_id !== existingBooking.cabin_id && !isAdmin) {
      return NextResponse.json(
        { error: "Only admins can change booking cabin" },
        { status: 403 },
      );
    }
    ...
    const finalGuests =
      number_of_guests !== undefined
        ? parseInt(number_of_guests.toString(), 10)
        : existingBooking.number_of_guests || 1;
    ...
    if (isOwner && !isAdmin && (datesChanged || guestsChanged)) {
      ...
      if (status === "approved") {
        ...
        if (cabinData && cabinData.nightly_fee > 0) {
          ...
          const totalAmount =
            nights * parseFloat(cabinData.nightly_fee.toString());
    ...
    if (number_of_guests !== undefined)
      updateData.number_of_guests = finalGuests;
