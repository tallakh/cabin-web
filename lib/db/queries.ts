...
export async function createUserProfile(userId: string, email: string, fullName: string): Promise<UserProfile> {
  const supabase = await createClient()
  const { data, error } = await supabase
    .from('user_profiles')
    .insert({
...
export async function getBookingsForDateRange(cabinId: string, startDate: string, endDate: string): Promise<Booking[]> {
  const supabase = await createClient()
  const { data, error } = await supabase
    .from('bookings')
    .select('*')
...
export async function getBookingStatistics(year?: number): Promise<BookingStats[]> {
  const supabase = await createClient()
  ...
  for (const booking of bookings) {
    ...
    // Calculate nights (inclusive of both start and end dates)
    const startDate = new Date(booking.start_date)
    const endDate = new Date(booking.end_date)
